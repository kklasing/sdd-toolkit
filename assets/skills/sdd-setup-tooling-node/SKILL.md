...
9. **Existing config** — never clobber. If a config already exists (ESLint, Prettier, Vitest, etc.), show the user and ask whether to merge, replace, or skip that piece.
10. **Missing prerequisites** — if `typescript`, a `tsconfig.json`, or a `package.json` is missing, stop and ask before continuing.
...
9. **Verify** (below), then tell the user what was set up and what they should review.
