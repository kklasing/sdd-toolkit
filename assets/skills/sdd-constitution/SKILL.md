...
- **Gather sources.** Skim the repo for rules it already implies — `README`, `docs/adr/`, `CONTEXT.md`, the CI workflow, lockfile/allowlist conventions, the PR template's attestation. These seed the rules so you interview the user about the *gaps*, not the obvious.
